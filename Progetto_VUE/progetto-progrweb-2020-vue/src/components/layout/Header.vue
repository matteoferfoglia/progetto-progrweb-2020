<template>
  ...
  <header>
    <nav>
      ...
      <router-link :to="{name: NOME_ROOT_APPLICAZIONE}">
        <img class="logo" :src="URL_LOGO_APPLICAZIONE" alt=""/>
        {{NOME_APPLICAZIONE}}
      </router-link>

      <div id="containerOpzioniUtente" v-if="isAutenticato()">
        ...
        <ul id="elencoOpzioniUtenteDaHeader">

          <li>
            <router-link :to="{path: PERCORSO_IMPOSTAZIONI_ACCOUNT}">
              <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-gear-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><!-- TODO : icone da aggiungere via CSS -->
          ...
          <li>
            <FormConCsrfToken @csrf-token-ricevuto="$emit('csrf-token-ricevuto',$event)"
              ...
              <a @click.prevent="$emit('logout')">
                <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-door-open" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><!-- TODO : icone da aggiungere via CSS -->
